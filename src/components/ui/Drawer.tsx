...
// SmartStock Experience V1 — Right-Side Operational Detail Drawer with Mobile Responsive View
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
        ...
        >
          {/* Header */}
          <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-[#E4E7EC] flex items-start justify-between bg-[#F9FAFB]">
            <div className="space-y-1 pr-2">
              <div className="flex items-center gap-2">
                {badge}
                <span className="text-[11px] font-mono text-[#667085]">SmartStock Detail</span>
              </div>
              <h2 className="text-sm sm:text-base font-semibold text-[#101828] tracking-tight">{title}</h2>
              {subtitle && <p className="text-xs text-[#475467]">{subtitle}</p>}
            </div>

            <button
              onClick={onClose}
              className="p-1.5 rounded-[6px] text-[#98A2B3] hover:text-[#344054] hover:bg-[#F2F4F7] transition-colors shrink-0"
          ...
          {/* Footer Actions */}
          {footer && (
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-[#E4E7EC] bg-[#F9FAFB] flex items-center justify-end gap-2 sm:gap-3">
              {footer}
            </div>
          )}
        </div>
